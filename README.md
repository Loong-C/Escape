...
Escape 是一款 11 × 11 的双人抽象策略游戏。玩家在交点落桩，相邻同色桩自动形成墙，并通过改变球四个相邻位置的逃生长度推动球离开棋盘。
...
- 简单难度在球的上、右、下、左四个相邻位置显示最短逃生路径长度；被墙挡住的方向显示 ∞，只有数值改变时才并列显示旧值与新值。若确认落桩后球会移动，还会显示虚线方向与落点轮廓，不显示完整路径。
