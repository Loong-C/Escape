...
Escape 是一款 17 × 17 的双人抽象策略游戏。玩家在交点落桩，相邻同色桩自动形成墙，并通过改变球四个相邻位置的逃生长度推动球离开棋盘。

线上版本：[linkukai.com/games/Escape](https://linkukai.com/games/Escape/)

## 功能

- 七步交互教程，依次讲解落桩、浮桩与锚桩、墙、替换浮桩、相邻位置的最短路径长度、球的自动移动、边界归属和封闭胜利。
- 支持人机对战与同设备轮流落桩的本地双人对战；人机模式随机分配黑白方，白方先行。
- 简单难度只在球的上、右、下、左四个相邻格中心显示最短逃生路径长度；预览落子时数字直接切换为新值，且仅在最小值唯一时以强调色标出。
- 困难难度隐藏逃生长度提示；两档使用完全相同的 AI 与搜索预算。
- 当前行动方所属的两条目标边会以克制的光带提示，并在回合交替时切换。
...
模型从零开始使用自我对弈 Monte Carlo 终局回报、经验回放、Adam 优化和逐步收敛的探索率训练。每个局面同时记录行动方与等待方视角；现有生产模型的初始课程从 3 × 3、5 × 5 的高密度封闭局面逐步过渡到 11 × 11，运行时特征会按当前 17 × 17 棋盘归一化。
